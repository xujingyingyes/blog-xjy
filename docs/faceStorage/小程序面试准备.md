...
* pages字段  —— 用于描述当前小程序所有页面路径，这是为了让微信客户端知道当前你的小程序页面定义在哪个目录。编译时显示第一个路径的页面。
...
* wx.switchTab()：跳转到 abBar 页面，并关闭其他所有非 tabBar 页面  `<navigator open-type="switchTab"/> `或用户切换 Tab
